...
R_comp <- R_comp %>% filter(!grepl(".Diff", rownames(R_comp)))
R_comp$stats <- rownames(R_comp) 
R_comp <- R_comp %>% select(stats, everything())

sheep <- R_comp %>% pivot_longer(cols = c("y_stats", "R_stats"))
sheep <- sheep %>% filter(stats %in% c("PTS", "TRB", "AST"))
sheep$name <- ifelse(sheep$name == "R_stats", sheep$name <- "Raptors", sheep$name <- "Non-Avg Team")

ggplot(data = sheep, aes(x = stats, y = value, fill = name)) + geom_bar(stat = "identity", position = position_dodge(), colour = "black") + scale_fill_manual(values = c("#0000FF", "#ce1100")) + ggtitle("Non-Average Team vs. Raptors (53-19) Comparison") + theme(axis.title.x=element_blank(), axis.title.y=element_blank())
```
